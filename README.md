...
> - 🌑 **Built-in display off** — fully turns off the MacBook's built-in display while the lid stays open (open-lid "clamshell", e.g. for cooling): a recorded ⌃⇧⌥-style combo or the menu bar, with an automatic emergency re-enable when the last external display is unplugged.
...
| | Version | Signed |
|---|---|---|
| **This fork** (+ keyboard backlight, external brightness, volume feedback, modifier keys, built-in display off) | [v3.7.0](https://github.com/0x3654/better-osd/releases/latest/download/BetterOSD-arm64.dmg) | ✗ no certificate |
